...
First we'll tweak our directory structure slightly, separating the "source" code (`./src`) from our "distribution" code (`./dist`).Separating source and distribution code helps keep development files clean while ensuring only optimized files are served to the browser in production.
